...
function RankBadge({ rank }: { rank: number }) {
  if (rank === 1) return <div className="w-8 h-8 rounded-full bg-gradient-to-br from-yellow-400 to-amber-500 flex items-center justify-center"><Crown size={14} className="text-white" /></div>;
...
function exportCSV(entries: LeaderboardEntry[], eventMode: boolean) {
  const headers = eventMode
    ? ["Rank", "Username", "Email", "Persona", "Level", "Stage", "Total Score", "Missions", "Avg Accuracy", "Time Spent (s)", "Wallet"]
    : ["Rank", "Username", "Email", "Persona", "XP", "Level", "Stage", "Missions", "Total Score", "Streak", "Wallet"];
  const rows = entries.map((e) =>
    eventMode
      ? [e.rank, e.username, e.email ?? "", e.persona ?? "", e.level, e.stage, e.totalScore, e.missionCount, e.avgAccuracy ?? 0, e.totalTimeSpent ?? 0, e.walletAddress ?? ""]
...
export default function SuperLeaderboardPage() {
  ...
  const [selectedEventId, setEvent]   = useState("");
  const [personaFilter, setPersona]   = useState("all");
  ...
  const { data: eventList = [] } = useQuery<EventOption[]>({
    queryKey: ["super-events-picker"],
    queryFn: () => fetch("/api/super/events").then((r) => r.json()),
    staleTime: 60_000,
  });
  ...
  const { data: rawEntries = [], isLoading, refetch, isFetching } = useQuery<LeaderboardEntry[]>({
    queryKey: ["super-leaderboard", mode, selectedEventId, personaFilter, stageFilter, deferredSearch],
    queryFn: () => fetch(`/api/super/leaderboard?${params}`).then((r) => r.json()),
    staleTime: 30_000,
  ...
  const selectedEvent = (eventList as any[]).find((e: any) => e.id === selectedEventId);
  ...
  const SortHeader = ({ col, label }: { col: typeof sortCol; label: string }) => (
    <button
      ...
      className={cn("flex items-center gap-1 text-xs font-semibold uppercase tracking-wider transition-colors", sortCol === col ? "text-arena-gold" : "text-slate-500 hover:text-slate-300")}
    >
      {label}
    ...
    <div className="p-6 space-y-6 text-white">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold text-arena-gold uppercase tracking-wider">Leaderboard</span>
          </div>
          <h1 className="text-3xl font-black text-white">
            {mode === "event" && selectedEvent ? `${(selectedEvent as any).cover_emoji ?? "🎮"} ${(selectedEvent as any).title}` : "Global Leaderboard"}
          </h1>
      ...
      <div className="flex items-center gap-2 p-1.5 glass rounded-2xl w-fit">
        <button
          ...
          className={cn("flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200", mode === "global" ? "bg-arena-gold text-black shadow" : "text-slate-400 hover:text-white")}
        >
          <TrendingUp size={15} />
          Global
        </button>
        <button
          ...
          className={cn("flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200", mode === "event" ? "bg-rose-600 text-white shadow" : "text-slate-400 hover:text-white")}
        >
          <Calendar size={15} />
          By Event
        </button>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
                {(eventList as any[]).length === 0 && <p className="text-slate-500 text-sm col-span-full">No events found.</p>}
                {(eventList as any[]).map((ev: any) => (
                  <button
                    key={ev.id}
                    ...
                    className={cn(
                      ...
                      selectedEventId === ev.id
                        ? "border-arena-gold/50 bg-arena-gold/10 text-white"
                        : "border-white/[0.06] bg-white/[0.02] text-slate-300 hover:bg-white/[0.05] hover:border-white/15"
                    )}
                  >
                    <span className="text-xl">{ev.cover_emoji ?? "🎮"}</span>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold truncate">{ev.title}</p>
                      <span className={cn("text-xs", ev.status === "live" ? "text-rose-400" : "text-slate-500")}>
          ...
          {PERSONAS.map((p) => (
            <button key={p} onClick={() => setPersona(p)}
              className={cn("px-3 py-1.5 rounded-lg text-xs font-medium transition-all capitalize whitespace-nowrap",
                personaFilter === p ? "bg-arena-gold text-black" : "border border-white/[0.08] text-slate-400 hover:text-white hover:border-white/20")}
            >{p === "all" ? "All personas" : p}</button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {STAGES.map((s) => (
            <button key={s} onClick={() => setStage(s)}
              className={cn("px-3 py-1.5 rounded-lg text-xs font-medium transition-all capitalize whitespace-nowrap",
                stageFilter === s ? "bg-rose-600 text-white" : "border border-white/[0.08] text-slate-400 hover:text-white hover:border-white/20")}
            >{s === "all" ? "All stages" : `${STAGE_EMOJIS[s]} ${s}`}</button>
          ))}
        </div>
      </div>
      ...
      {entries.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Players ranked", value: entries.length, icon: Users, color: "bg-rose-500/20" },
            { label: "Top XP", value: topXP.toLocaleString(), icon: Zap, color: "bg-arena-gold/20" },
            { label: "Average XP", value: avgXP.toLocaleString(), icon: TrendingUp, color: "bg-purple-500/20" },
            { label: "Total missions", value: totalMissions.toLocaleString(), icon: Target, color: "bg-cyan-500/20" },
          ...
          <div className="divide-y divide-white/[0.04]">
            {entries.map((entry, idx) => (
              <motion.div
                ...
                className={cn(
                  "grid grid-cols-[auto_1fr_auto_auto_auto_auto_auto] gap-x-4 px-4 py-3 items-center hover:bg-white/[0.03] transition-colors",
              ...
              >
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {entry.statusTag && (
                        <span className="text-xs text-amber-400 bg-amber-400/10 border border-amber-400/20 px-1.5 py-0.5 rounded-md">{entry.statusTag}</span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {entry.email && (
                        <span className="text-xs text-slate-600 truncate max-w-40">{entry.email}</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="text-right">
                  <p className={cn("font-bold text-sm tabular-nums", entry.rank === 1 ? "text-amber-400" : "text-amber-400/70")}>{entry.xp.toLocaleString()}</p>
                  <p className="text-slate-600 text-xs">XP</p>
                </div>
                <div className="text-right">
                  <p className="text-cyan-400 font-bold text-sm tabular-nums">{(entry.totalScore ?? 0).toLocaleString()}</p>
                  {mode === "event" && entry.avgAccuracy != null && <p className="text-slate-600 text-xs">{entry.avgAccuracy}% acc</p>}
                </div>
                <div className="text-right">
                  <p className="text-white font-semibold text-sm tabular-nums">{entry.missionCount}</p>
                  {mode === "event" && entry.totalTimeSpent != null && <p className="text-slate-600 text-xs">{Math.round(entry.totalTimeSpent / 60)}m</p>}
                </div>
                <div className="text-right">
